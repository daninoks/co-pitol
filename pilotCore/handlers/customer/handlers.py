...
import re
import sys
from telegram import ParseMode, Update, ForceReply, ReplyKeyboardRemove, Bot
...
from pilotCore.models import User, Customer, CustomerRides, Driver, DriverRides
...
def update_availible_routes_list(update) -> list:
    ...
    availible_routes_open = list(
        DriverRides.objects.filter(status=DriverRides.RIDE_OPEN).values(
            "user_id",
            "ride_id",
            "departure_time",
            "direction",
...
def customer_properties(update: Update, context: CallbackContext) -> int:
    ...
    u = User.get_user_by_username_or_user_id(c.user_id)
...
def customer_actions_set(
    update: Update, context: CallbackContext, field_name: str
) -> None:
    ...
    text = customer_text.customer_properties_set_dt.get(
        updating_field.get("name")
    ).format(cb_var=updating_field.get("data"))
...
def customer_list_routes(update: Update, context: CallbackContext) -> int:
    ...
    context.bot.edit_message_text(
        ...
        chat_id=update.callback_query.message.chat.id,
        message_id=update.callback_query.message.message_id,
        reply_markup=customer_keyboards.make_keyboard_customer_list_routes(
            rides_exists, pages_layout
        ),
        parse_mode=ParseMode.HTML,
    )
    return conversation.MAIN_TREE
...
def customer_select_seats(update: Update, context: CallbackContext) -> int:
    ...
    u, _ = User.get_user_and_created(update, context)
    cr, _ = CustomerRides.get_or_create_user(update, context)
    dr = DriverRides.get_dr_by_ride_id(u.selected_ride_id)
    d = Driver.get_d_by_user_id(dr.user_id)
    ...
    if call_back == customer_data.CUSTOMER_SELECT_SEATS_CONFIRM_CB:
        ...
        cr.ride_id_booked = u.selected_ride_id
        cr.save()
