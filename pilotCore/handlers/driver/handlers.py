...
from pilotCore.models import User, Driver, DriverRides
...
def get_ride_list(update) -> list:
    ...
    rides_list = list(
        DriverRides.objects.filter(
            user_id=update.callback_query.message.chat.id, status=DriverRides.RIDE_OPEN
        ).values(
            "user_id",
            "ride_id",
            "departure_time",
    ...
    if d.registred == d.DRVR_ACCEPTED:
        ...
        for item in my_rides:
            my_rides_block += driver_text.my_rides_overiew.format(
                ...
                booked_seats=(
                    0 if item.get("seats_booked") == None else item.get("seats_booked")
                ),
        ...
        text = driver_text.driver_main_text.format(
            ...
            tel_number="-" if d.mobile_number is None else f"+{d.mobile_number}",
            seats="-" if d.car_seats is None else d.car_seats,
            rides_overview="" if my_rides_block == "" else my_rides_block,
        )
        keyboard = driver_keyboard.make_keyboard_driver_main()
    ...
    if my_rides:
        ...
        if call_back in driver_data.MR_DYNAMIC_CB_RIDE:
            ...
            current_page = User.set_myride_page(
                u, pages_max, int(re.sub(f"{driver_data.MR_CB_PREFIX}:", "", call_back))
            )
        # keyboard pages:
        ...
        u.selected_ride_id = rideObj.get("ride_id")
        u.save()
        ...
        text = driver_text.my_rides_text.format(
            ...
            booked_seats=(
                0
                if rideObj.get("seats_booked") == "None"
...
def my_rides_time(update: Update, context: CallbackContext) -> int:
    ...
    text = f"<code>Selected time: {dr_time}</code>"
...
def set_direction(update: Update, context: CallbackContext) -> int:
    ...
    driver_obj, _ = Driver.get_or_create_user(update, context)
    d = DriverRides.get_dr_by_ride_id(driver_obj.new_ride_id)
    ...
    if call_back in driver_data.CITIES_CALLBACK:
        ...
        d = DriverRides.add_direction(update, context, field_data)
        text = d.direction
...
def driver_preference(update: Update, context: CallbackContext) -> int:
    ...
    Entry point to: HOURS_CONV, MODEL_CONV, SEATS_CONV, COLOR_CONV, NUMBER_CONV, MOBILE_NUMBER_CONV
    """
    d, _ = Driver.get_or_create_user(update, context)
    call_back = update.callback_query.data
    ...
    if call_back in driver_data.REPLY_HANDLESR:
        if getattr(d, call_back_sub) is None:
            ...
            if call_back_sub == "mobile_number":
                text = driver_text.driver_preference_text[call_back_sub].format(
                    cb_var=f"+{getattr(d, call_back_sub)}"
                )
            else:
                text = driver_text.driver_preference_text[call_back_sub].format(
...
def bot_actions_set(update: Update, context: CallbackContext, field_name: str) -> None:
    ...
    updating_field = {"name": field_name, "data": update.message.text}
